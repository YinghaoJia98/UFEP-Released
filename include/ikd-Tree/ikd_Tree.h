...
struct ikdTree_PointType_new
{
    ...
    // void *data;
    //  wgh 为什么这里不用const&做形参？
    ikdTree_PointType_new(float px = 0.0f, float py = 0.0f, float pz = 0.0f)
    {
        x = px;
        y = py;
        z = pz;
...
private:
    ...
    pthread_mutex_t termination_flag_mutex_lock, rebuild_ptr_mutex_lock, working_flag_mutex, search_flag_mutex, Rebuild_Ptr_For_Change_And_Use_mutex_lock;
